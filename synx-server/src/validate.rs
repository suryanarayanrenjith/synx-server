//! Deciding whether a claimed position is a position a car could be in.
//!
//! # The model
//!
//! The server does not run the solver. It could - it is a Rust crate sitting
//! right there - and it deliberately does not, for two reasons that are worth
//! stating because "the server should simulate everything" is the received
//! wisdom and it is wrong here.
//!
//! IT WOULD NOT AGREE WITH THE CLIENT ANYWAY. The solver calls `sin`, `cos`
//! and `exp` in its inner loop. Compiled to WebAssembly those come from Rust's
//! own `libm`; compiled for the server they come from the platform's. The two
//! differ in the last place, and over a four-minute race a last-place
//! difference in a feedback loop is metres. A server that re-simulates would
//! therefore disagree with an honest client constantly, and would have to
//! correct it constantly - which is a worse experience than the thing it was
//! trying to prevent, and which would hide real cheating in the noise.
//!
//! IT WOULD NOT FIT. A four-wheel solver at 60 Hz for four cars in each of a
//! dozen rooms is two and a half thousand full physics steps a second, which
//! is not a reasonable price for deciding whether somebody is cheating.
//!
//! So the server checks an ENVELOPE instead: not "is this exactly where the
//! physics would have put you" but "is this inside the set of places the
//! physics could have put you". That is a much cheaper question - a few dozen
//! floating point operations and one centreline projection - and for a racing
//! game it closes every exploit that matters, because all of them are outside
//! the envelope by a mile rather than by a millimetre:
//!
//! | exploit                     | caught by                                |
//! |-----------------------------|------------------------------------------|
//! | speed hack                  | the ceiling, which the solver hard-clamps |
//! | teleport / warp to finish   | displacement against real elapsed time    |
...
//! | rewinding to hide a mistake | monotonic timestamps                      |
//! | inflating the frame delta   | elapsed time bounded by the wall clock    |
//!
//! # The one number that makes it work
//!
//! `dt` - how much time a client is allowed to claim has passed - is taken as
//! the SMALLER of what the client says and what the server's own clock
//! measured. Without that, every distance test is defeated by claiming a large
//! delta; with it, a client cannot buy a metre of movement it has not waited
//! for. Everything else here is a consequence of that.
